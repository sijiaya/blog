...
public class Mail {
	public Mail(User user) {
		try {
			...
			//  设置发信人地址
			message.setFrom(new InternetAddress("2317321437@qq.com"));
			// 设置收信人地址
			...
			// 设置e-mail内容
			message.setContent(user.getMobile()+"请经常访问本网站！" +
					"  <br><a href='http://localhost:8080/board/email?id="+user.getId()+"&status="+user.getStatus()+"'>请点击激活帐号</a>", "text/html;charset=UTF-8");
            ...
            //"<a href='#' onclick='javascript:goToClient();'>请点击激活帐号ʺ</a> ", "text/html;charset=gbk");
			// 保存对于Email.的修改
			message.saveChanges();
			// 根据Session生成Transport对象
			transport = sendsession.getTransport("smtp");
			// 连接到SMTP服务器
			transport.connect("smtp.qq.com", "2317321437", "nghgutqgmixuecig");
			// 发送e-mail
